...
function poseLinesWithZ(pointResolver, pointZ, strokeWidth, strokeColor = "black", drawOutline = true, jointMaskRadius = null) {
  ...
  for (let i = 0; i < lineDefs.length; i += 1) {
    ...
    segments.push({
      ...
      markup: [
        drawOutline
          ? `<line x1="${start.x}" y1="${start.y}" x2="${end.x}" y2="${end.y}" stroke="black" stroke-width="${strokeWidth + 2}" stroke-linecap="round" stroke-linejoin="round"/>`
          : "",
        `<line x1="${start.x}" y1="${start.y}" x2="${end.x}" y2="${end.y}" stroke="${strokeColor}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round"/>`,
      ].join(""),
    });
  }
